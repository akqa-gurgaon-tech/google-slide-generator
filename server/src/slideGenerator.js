...
export async function createPresentation(slides, slidesData, presentationId) {
        ...
        if (slide.theme) {
            ...
            placeholders?.forEach((placeholder) => {
                if (placeholder.shape?.placeholder?.type) {
                    ...
                    const hasContent = slide.inputs && ((placeholderType === 'CENTERED_TITLE' && slide.inputs.CENTERED_TITLE) ||
...
export async function createEmptyPresentation(slidesApi, presentationTitle = "Untitled presentation") {
...
export async function deleteAllSlides(presentationId, slides) {
    // Step 1: Get all slide IDs
    const presentation = await slides.presentations.get({ presentationId });
    const slideIds = presentation.data.slides.map((slide) => slide.objectId);
...
export async function updatePresentationTitle(client, presentationId, title) {
...
function getTextTypeFromPlaceholder(placeholderType) {
...
//# sourceMappingURL=slideGenerator.js.map