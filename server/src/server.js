...
import { createPresentation, createEmptyPresentation, updatePresentationTitle, } from "./slideGenerator.ts";
...
app.put("/api/themes/:themeId", async (req, res) => {
    ...
    try {
        const updates = {};
        if (theme_desc !== undefined)
            updates.theme_desc = theme_desc;
        if (schema_str !== undefined)
            updates.schema_str = schema_str;
