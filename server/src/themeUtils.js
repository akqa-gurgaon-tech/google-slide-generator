...
export function applyThemesToSlides(slides, themes) {
    if (!themes) {
        return slides;
    }
...
export function generateSlideStyleRequests(theme) {
    const requests = [];
...
export function generateTextStyleRequests(theme, textType) {
...
export function createThemeColorPalette(theme) {
    const colors = [];
    if (theme.colors?.accents) {
        Object.values(theme.colors.accents).forEach((color) => {
...
export function applyThemeToMasterLayouts(theme) {
    const requests = [];
